...
class LoRAModule(torch.nn.Module):
    ...
    def __init__(
        ...
        # 指定lora的B矩阵数量
        self.lora_ups_num = lora_ups_num
        # self.lora_category_record = []
        ...
        if split_dims is None:
            if org_module.__class__.__name__ == "Conv2d":
                ...
                # self.lora_ups = [torch.nn.Conv2d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False) for _ in range(self.lora_ups_num)]
                self.lora_ups = torch.nn.ModuleList([
                    torch.nn.Conv2d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False) for _ in range(self.lora_ups_num)
                ])
            else:
                self.lora_down = torch.nn.Linear(in_dim, self.lora_dim, bias=False)
                # self.lora_ups = [torch.nn.Linear(self.lora_dim, out_dim, bias=False) for _ in range(self.lora_ups_num)]
                self.lora_ups = torch.nn.ModuleList([
                    torch.nn.Linear(self.lora_dim, out_dim, bias=False) for _ in range(self.lora_ups_num)
                ])

            torch.nn.init.kaiming_uniform_(self.lora_down.weight, a=math.sqrt(5))   #  Kaiming Uniform 初始化lora_down.weight
            # for lora_down in self.lora_downs:
            #     torch.nn.init.kaiming_uniform_(lora_down.weight, a=math.sqrt(5))
            # self.lora_downs = [lora_down.to("cuda") for lora_down in self.lora_downs]
            for lora_up in self.lora_ups:
                torch.nn.init.zeros_(lora_up.weight)   # 零初始化lora_up.weight
    ...
    def forward(self, x):
        ...
        # logging.info(f"using set_lora_down_cur: {self.lora_down_cur}")
        ...
        if self.split_dims is None:
            ...
            # 冻结 lora_ups 中除了 lora_up_cur 之外的其他层
            for i, lora_up in enumerate(self.lora_ups):
                if i != self.lora_up_cur:
                    for param in lora_up.parameters():
                        param.requires_grad = False  # 冻结其他层
                else:
                    for param in lora_up.parameters():
                        param.requires_grad = True  # 启动当前层

            # 使用当前选择的 lora_up_cur 进行前向传播
            lx = self.lora_ups[self.lora_up_cur](lx)
...
class LoRAInfModule(LoRAModule):    # 专门用于推理，舍弃了训练时的dropout
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(lora_name, org_module, multiplier, lora_dim, alpha, lora_ups_num=lora_ups_num)
    ...
    def get_weight(self, multiplier=None):
        ...
        up_weight = self.lora_ups[self.lora_up_cur].weight.to(torch.float)
...
def create_network_from_weights(multiplier, file, ae, text_encoders, flux, weights_sd=None, for_inference=False, lora_ups_num=1, **kwargs):
...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.unet_loras, skipped_un = create_modules(True, None, unet, target_replace_modules, lora_ups_num=lora_ups_num)     # skipped_un: []
